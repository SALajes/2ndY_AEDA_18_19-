...
{
	return os << car.brand << " " << car.model << " (" << car.price << ")";
}
...
FuelCar::FuelCar(string brand, string model,float price,float tank, float l_100km) : Car(brand, model), tank(tank), l_100km(l_100km)
{
	setPrice(price);
}
...
ElectricCar::ElectricCar(string brand, string model, float price, float battery, float kWh_100km) : Car(brand,model), battery(battery), kWh_100km(kWh_100km)
{
	setPrice(price);
}
...
{
	return ElectricCar::range() + FuelCar::range();
}
